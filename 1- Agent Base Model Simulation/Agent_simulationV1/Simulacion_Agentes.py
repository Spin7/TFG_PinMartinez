...
# ==============================
class Environment:
    def __init__(self, urban_grid_path, 
                 delta_time,
                 grid_to_meters,
                 num_breeding_sites=15, 
                 K_per_site=100,
                 base_potential=0.5, 
                 lambda_decay=6):
        
        self.delta_time = delta_time #En dias
        ...
        self.R_cut_off = 50*self.delta_time/self.grid_to_meters
    ...
    def _downsample_grid(self, urban_grid_path):
        """Carga la grilla original"""
        original_grid = np.load(urban_grid_path)
        downsampled = original_grid
        return np.array(downsampled)


    def _place_breeding_sites(self, num_sites):
        # --- 1) base mask: permitido ---
        allowed = [0, 2, 4]  # empty space, tree, bulding
        forbidden = [1, 3]  # roads, water
    ...
    def _compute_potential_field(self):
        ...
        # Inicializar distancia mínima a criaderos
        ...
        potential[min_dist > self.R_cut_off] = 0.0

        #  Normalización global del potencial (0–1)
    ...
    # ------------------------------
    def get_possible_moves(self, pos, movement_range_in_meters):
        """Devuelve lista de posiciones vecinas y su potencial"""
        movement_range = (movement_range_in_meters*self.delta_time)/self.grid_to_meters
        y, x = pos
        ...
        candidates = []
        for ny in range(int(y_min), int(y_max)):
            for nx in range(int(x_min), int(x_max)):
                features = {
    ...
    def show_potentialfield(self, cmap="plasma", interpolation="gaussian", grid=True, annotate=False):
        """
        Visualiza el campo de potencial como un mapa de calor suave.

        Parámetros:
        ------------
        cmap : str
            Colormap a usar (ej: 'viridis', 'plasma', 'inferno', 'magma', 'turbo').
        interpolation : str
            Tipo de suavizado: 'none', 'bilinear', 'gaussian' o 'nearest'.
        grid : bool
            Si es True, muestra una cuadrícula ligera para las celdas.
        annotate : bool
            Si es True, muestra el valor numérico del potencial en cada celda.
        """
        import matplotlib.pyplot as plt
        import numpy as np

        if not hasattr(self, "potential_field"):
            print("El campo de potencial no ha sido calculado todavía.")
            return

        fig, ax = plt.subplots(figsize=(8, 6))

        # Normalización y visualización suave
        im = ax.imshow(
        ...
        cbar.set_label("Potencial", fontsize=11)

        # Marcar los criaderos
        if hasattr(self, "breeding_sites") and self.breeding_sites:
            ys, xs = zip(*self.breeding_sites)
            ax.scatter(xs, ys, color="red", s=50, marker="x", label="Criaderos")

        # Mostrar cuadrícula si se desea
        if grid:
            ax.set_xticks(np.arange(-0.5, self.potential_field.shape[1], 1), minor=True)
            ax.set_yticks(np.arange(-0.5, self.potential_field.shape[0], 1), minor=True)
            ax.grid(which="minor", color="gray", linestyle=":", linewidth=0.5)
            ax.tick_params(which="minor", size=0)

        # Opcional: mostrar valores numéricos en cada celda
        if annotate:
            for i in range(self.potential_field.shape[0]):
                for j in range(self.potential_field.shape[1]):
                    val = self.potential_field[i, j]
                    ax.text(j, i, f"{val:.1f}", ha="center", va="center", color="white", fontsize=7)

        ax.set_title("Mapa de calor del campo de potencial", fontsize=13)
...
class Adult(Agent):
    ...
    def move(self, env: Environment, delta_time, grid_to_meters):
        """El adulto decide movimiento usando potencial del entorno"""
        movement_range_in_meters = 50
        candidates = env.get_possible_moves(self.pos, movement_range_in_meters)
...
class MosquitoSimulation:
    def __init__(self, env: Environment, delta_t, grid_to_meters):
        self.env = env
    ...
    def step(self):
        ...
        for a in adults_list:
            a.move(self.env, self.delta_t, self.grid_to_meters)    # mueve al adulto (actualiza su .pos)
    ...
    # ======================== FUNCION DE TRACKING PARA LAS ESTADISTICAS ========================
    ...
    def visualize_map_simulation_at_step(self, step):
        ...
        cmap = ListedColormap([
                [0.9, 0.9, 0.9], # Vacio - Crema
                [93/255, 98/255, 99/255], # Road - Griz
                [0.0, 0.6, 0.0], # Vegetation - Verde
                [0.0, 0.0, 0.8], # Agua - Azul
                [0.5, 0.3, 0.1], # Construciones - Marron
            ])
        plt.imshow(self.env.urban_grid, cmap=cmap, vmin=0, vmax=5)
    ...
    def run(self, t_max = 500, visualize_intervals_steps = [0, 100, 250, 500]):
...
if __name__ == "__main__":
    # Parametros del entorno y simulacion
    delta_t = 1
    grid_to_meters = 1
    num_breeding_sites=15
    K_per_site=100
    ...
    env = Environment("grid_data.npy", delta_t, grid_to_meters, num_breeding_sites, K_per_site)
    ...
    def run_multiple_times(simulation_runs, t_max=500, J0=60, A0=40):
        ...
        for run in range(simulation_runs):
            sim = MosquitoSimulation(env, delta_t, grid_to_meters)
    ...
    sim = MosquitoSimulation(env, delta_t, grid_to_meters)
    sim.initialize_population(60, 40)
    sim.run()
